...
from cliutils import (Cmd, execute, node_output_ok, node_output_notok,
                      sync_file_to_peers, GlusterCmdException,
                      output_error, CliTable, execute_in_peers, runcli)

from events.eventsapiconf import WEBHOOKS_FILE_TO_SYNC, WEBHOOKS_FILE, \
    DEFAULT_CONFIG_FILE, CUSTOM_CONFIG_FILE, CUSTOM_CONFIG_FILE_TO_SYNC, \
    EVENTSD, CONFIG_KEYS, BOOL_CONFIGS, RESTART_CONFIGS
...
def sync_to_peers(restart=False):
    ...
    table = CliTable(num_columns=3)
    table.format_column(2, align="right", color=color_status)
    table.format_column(3, align="right", color=color_status)
    table.add_title_row("NODE", "NODE STATUS", "SYNC STATUS")
    for p in out:
...
def action_handle(action):
    ...
    table.format_column(2, align="right", color=color_status)
    table.format_column(3, align="right", color=color_status)
    table.add_title_row("NODE", "NODE STATUS", column_name + " STATUS")
    for p in out:
...
class WebhookTestCmd(Cmd):
    ...
    def run(self, args):
        ...
        table.format_column(2, align="right", color=color_status)
        table.format_column(3, align="right", color=color_status)
        table.add_title_row("NODE", "NODE STATUS", "WEBHOOK STATUS")
        for p in out:
            table.add_row(p.hostname,
                          "UP" if p.node_up else "DOWN",
                          "OK" if p.ok else "NOT OK: {0}".format(
...
class ConfigGetCmd(Cmd):
    ...
    def run(self, args):
        ...
        table = CliTable(num_columns=2)
        table.add_title_row("NAME", "VALUE")
