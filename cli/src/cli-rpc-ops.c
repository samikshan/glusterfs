...
cli_print_volume_status_clients (dict_t *dict, gf_boolean_t notbrick)
{
        ...
        for (i = 0; i <= index_max; i++) {
                ...
                cli_out ("%-48s %15s %15s %15s %15s", "Hostname", "BytesRead",
                         "BytesWritten", "MinOpVersion", "MaxOpVersion");
                cli_out ("%-48s %15s %15s %15s %15s", "--------", "---------",
                         "------------", "------------", "------------");
                for (j =0; j < client_count; j++) {
                        ...
                        memset (key, 0, sizeof (key));
                        snprintf (key, sizeof (key),
                                 "brick%d.client%d.minopversion", i, j);
                        ret = dict_get_uint32 (dict, key, &minopversion);

                        memset (key, 0, sizeof (key));
                        snprintf (key, sizeof (key),
                                 "brick%d.client%d.maxopversion", i, j);
                        ret = dict_get_uint32 (dict, key, &maxopversion);

                        cli_out ("%-48s %15"PRIu64" %15"PRIu64
                                 " %15"PRIu32" %15"PRIu32, clientname,
                                 bytesread, byteswrite,
                                 minopversion, maxopversion);
                }
        }
out:
        cli_out ("----------------------------------------------\n");
