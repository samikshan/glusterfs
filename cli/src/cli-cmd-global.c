...
cli_cmd_get_state_cbk (struct cli_state *state, struct cli_cmd_word *word,
                              const char **words, int wordcount);
int
cli_cmd_get_max_opversion_cbk (struct cli_state *state,
...
struct cli_cmd global_cmds[] = {
        ...
        {
          "get-max-opversion",
           cli_cmd_get_max_opversion_cbk,
          "Get maximum supported op-version in cluster",
        },
        {NULL,  NULL,  NULL}
};
                               ...
                               int wordcount)
{
        ...
        ret = cli_cmd_get_max_opversion_parse (state, words, wordcount,
                                               &options, &op_errstr);
        ...
        proc = &cli_rpc_prog->proctable[GLUSTER_CLI_MAX_OPVERSION];
        if (proc->fn)
                ret = proc->fn (frame, THIS, options);
out:
        if (ret) {
                cli_cmd_sent_status_get (&sent);
                if ((sent == 0) && (parse_error == 0))
                        cli_out ("Getting maximum supported op-version failed");
        }
