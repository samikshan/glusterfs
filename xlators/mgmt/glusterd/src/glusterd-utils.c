...
glusterd_get_global_options_for_all_vols (dict_t *ctx, char **op_errstr)
{
        ...
        for (i = 0; valid_all_vol_opts[i].option; i++) {
                ...
                if ((!all_opts && strcmp (key, allvolopt) != 0) ||
                                        strcmp ("all", allvolopt) == 0)
                        continue;

                if (strcmp ("cluster.op-version", allvolopt) == 0) {
                        count++;
                        sprintf (dict_key, "key%d", count);
                        ret = dict_set_dynstr_with_alloc (ctx, dict_key, allvolopt);
                        if (ret) {
                                gf_msg (this->name, GF_LOG_ERROR, 0,
                                        GD_MSG_DICT_SET_FAILED, "Failed to set "
                                        "cluster.op-version in dictionary");
                                goto out;
                        }
                        ...
                        ret = dict_set_dynstr_with_alloc (ctx, dict_key, op_version_buff);
                        if (ret) {
                                gf_msg (this->name, GF_LOG_ERROR, 0,
                                        GD_MSG_DICT_SET_FAILED, "Failed to set value "
                                        "for key cluster.op-version in dictionary");
                                goto out;
                        }
