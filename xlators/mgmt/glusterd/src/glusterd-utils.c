...
my_callback (struct rpc_req *req, struct iovec *iov, int count, void *v_frame)
{
        ...
        if (req->rpc_status == 0) {
                /* On errors, it seems like this gets freed for us. */
                iobref_unref ((struct iobref *)(frame->local));
        }
              ...
              glusterd_volinfo_t *other_vol)
{
        ...
        snprintf (full_id, sizeof(full_id), "%s.%s.%s", volinfo->volname,
                                 ...
                                 glusterd_brickinfo_t *brickinfo)
{
        ...
        cds_list_for_each_entry (other_brick, &volinfo->bricks,
                ...
                /* TBD: do other compatibility checks (e.g. transport) here. */
                return other_brick;
        }
